    ...
    private fun initViewSubmit() {
        btn_reset.setOnClickListener { v ->
            ...
            if(pass.isEmpty()){
                ...
                hideSoftKeyboad(v)
                Utils.showToast(this@ResetPasswordActivity, getString(R.string.login_validation_valid), Color.RED)
                txt_et_new.requestFocus()
            ...
            override fun onResponse(call: Call<ResponseModel>?, response: Response<ResponseModel>?) {
                ...
                if (status != 200) {
                    when (status) {
                        201 -> {
                            val mess = response!!.body().message.toString()
                            Utils.showToast(this@ResetPasswordActivity, mess, Color.YELLOW)
                        }
                        204 -> Utils.showToast(this@ResetPasswordActivity,errorHandler(response), Color.YELLOW)
                        409 -> Utils.showToast(this@ResetPasswordActivity,errorHandler(response), Color.YELLOW)
                        400 -> Utils.showToast(this@ResetPasswordActivity,errorHandler(response), Color.YELLOW)
                        401 -> Utils.showToast(this@ResetPasswordActivity,errorHandler(response), Color.YELLOW)
                        403 -> Utils.showToast(this@ResetPasswordActivity,errorHandler(response), Color.YELLOW)
                        404 -> Utils.showToast(this@ResetPasswordActivity,errorHandler(response), Color.YELLOW)
                        500 -> Utils.showToast(this@ResetPasswordActivity,resources.getString(R.string.error_status_1), Color.YELLOW)
